 ...
 */
 ...
 public class Scheduler {
    ...
    private static List<String> topologicalSort(Graph graph) throws Exception {
        ...
        for (Node node : graph.getAllNodes()) {
            if (!visited.contains(node)) {
                if (topologicalSortUtil(node, visited, stack, recursionStack)) {
     ...
     * @param recursionStack a set to keep track of nodes in the current path
     * @throws Exception if an error occurs during sorting
     */
    private static boolean topologicalSortUtil(Node node, Set<Node> visited, Stack<Node> stack, Set<Node> recursionStack) throws Exception {
        visited.add(node);
        recursionStack.add(node);

        for (Node neighbor : node.getPrerequisites()) {
            if (!visited.contains(neighbor)) {
                if (topologicalSortUtil(neighbor, visited, stack, recursionStack)) {
                    return true;
                }
